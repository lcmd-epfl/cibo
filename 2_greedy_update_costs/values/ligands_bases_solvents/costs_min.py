...
if __name__ == "__main__":
    ...
    for exp_config in benchmark:
        ...
        for run in range(N_RUNS):
            ...
            LIGANDS_candidate_BO, BASES_candidate_BO,SOLVENTS_candidate_BO = cp.deepcopy(LIGANDS_candidate), cp.deepcopy(BASES_candidate),cp.deepcopy(SOLVENTS_candidate)
            LIGANDS_candidate_RANDOM,BASES_candidate_RANDOM, SOLVENTS_candidate_RANDOM = cp.deepcopy(LIGANDS_candidate), cp.deepcopy(BASES_candidate),cp.deepcopy(SOLVENTS_candidate)

    
            y_better_BO, y_better_RANDOM = [], []
            ...
            for i in range(NITER):
                if COST_AWARE_BO == False:
                    ...
                    suggested_costs_all, _ = compute_price_acquisition_all(
                        NEW_LIGANDS, NEW_BASES, NEW_SOLVENTS, price_dict_BO
                    )
                    y_best_BO = check_better(y, y_best_BO)
                    y_better_BO.append(y_best_BO)
                    running_costs_BO.append(
                        (running_costs_BO[-1] + suggested_costs_all)
                    )
                    ...
                    SOLVENTS_candidate_BO = np.delete(
                        SOLVENTS_candidate_BO, indices, axis=0
                    )
                    price_dict_BO = update_price_dict_all(
                        price_dict_BO, NEW_LIGANDS, NEW_BASES, NEW_SOLVENTS
                    )
                else:
                    SUCCESS_1 = False
                    indices, candidates = gibbon_search(
                    ...
                    while (cheap_indices == []) or (len(cheap_indices) < BATCH_SIZE):
                        ...
                        if (cheap_indices != []) and len(cheap_indices) == BATCH_SIZE:
                            ...
                            BASES_candidate_BO = np.delete(
                                BASES_candidate_BO, cheap_indices, axis=0
                            )
                            SOLVENTS_candidate_BO = np.delete(
                                SOLVENTS_candidate_BO, cheap_indices, axis=0
                            )
                            price_dict_BO = update_price_dict_all(
                                price_dict_BO,
                                NEW_LIGANDS[cheap_indices_1],
                                NEW_BASES[cheap_indices_1],
                                NEW_SOLVENTS[cheap_indices_1],
                            )
                ...
                suggested_costs_all, price_per_ligand = compute_price_acquisition_all(
                    NEW_LIGANDS, NEW_BASES, NEW_SOLVENTS, price_dict_RANDOM
                )
                ...
                running_costs_RANDOM.append(
                    running_costs_RANDOM[-1] + suggested_costs_all
                )
                ...
                BASES_candidate_RANDOM = np.delete(
                    BASES_candidate_RANDOM, indices_random, axis=0
                )
                SOLVENTS_candidate_RANDOM = np.delete(
                    SOLVENTS_candidate_RANDOM, indices_random, axis=0
                )
                price_dict_RANDOM = update_price_dict_all(
                    price_dict_RANDOM, NEW_LIGANDS, NEW_BASES, NEW_SOLVENTS
                )
