...
ax.plot(random_results_ebdo, label="Random", color="black", ls="--")
for i in range(6):
    ax.plot(all_bo_results_ebdo[i],
               label=f"Max $ per Batch: {i}", color=generate_color_scale(6)[i])
# Force integer x-axis labels.
ax.xaxis.set_major_locator(MaxNLocator(integer=True))
ax.set_xlabel("Iteration")
ax.set_ylabel("Yield (%)")  # Assuming yield is in percentage.
ax.set_title("EBDO Direct Arylation")
ax.legend()
