    ...
    for run in range(N_RUNS):
        ...
        for i in range(NITER):
            if COST_AWARE_BO == False:
                ...
                suggested_costs_all,_ = compute_price_acquisition_ligands(NEW_LIGANDS, price_dict_BO)
                ...
                indices, candidates = gibbon_search(model, X_candidate_BO,bounds_norm, q=BATCH_SIZE)
                NEW_LIGANDS = LIGANDS_candidate_BO[indices]
                suggested_costs_all, price_per_ligand = compute_price_acquisition_ligands(NEW_LIGANDS, price_dict_BO)
                ...
                while (cheap_indices ==[]) or (len(cheap_indices) < BATCH_SIZE):
                    ...
                    if (cheap_indices !=[]) and len(cheap_indices) == BATCH_SIZE:
                        ...
                        price_dict_BO        = update_price_dict_ligands(price_dict_BO, NEW_LIGANDS[cheap_indices_1])
            ...
            suggested_costs_all, price_per_ligand = compute_price_acquisition_ligands(NEW_LIGANDS, price_dict_RANDOM)
